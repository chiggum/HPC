...
rgb hsl2rgb(double h, double sl, double l)
{
    double v;
    double r,g,b;
...
rgb getColor(int n, int MAXCOLORS) {
...
complex<double> getFuncVal(complex<double> z, complex<double> c, int categ) {
	switch(categ) {
		case 1:
			return z*z+c;
		case 2:
...
void fractalForm(unsigned char *mat, int maxZAbs, int maxN, int minN, int decayN, double iReal,
...
int main(int argc, char **argv) {
	...
	for(double iReal = (interceptRealMin+interceptRealMax)/2, iImg = (interceptImgMin+interceptImgMax)/2;;) {
		start = clock();
		fractalForm(fImgData, maxZAbs, maxN, minN, decayN, iReal,
